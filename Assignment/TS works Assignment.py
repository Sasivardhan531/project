...
companies = ["AAPL", "GOOG", "MSFT", "AMZN", "IBM"]
...
import pandas as pd 
#connect to the database
connection = sqlite3.connect('google.db')
#extracting the dowloaded data
...
v = connection.cursor()
#query the database
v.execute("CREATE TABLE google (date integer,open integer,high integer, low integer, close integer, adjusted_close integer, volume integer, dividend_amount integer, split_coefficient integer)")
with open('GOOG.csv','r') as file:
    records = 0
    for row in file:
        v.execute("INSERT INTO google VALUES(?,?,?,?,?,?,?,?,?)",row.split(","))
        connection.commit()
        records +=1
...
import pandas as pd 
#connect to the database
connection = sqlite3.connect('ibm.db')
#extracting the dowloaded data
...
v = connection.cursor()
#query the database
v.execute("CREATE TABLE ibm (date integer,open integer,high integer, low integer, close integer, adjusted_close integer, volume integer, dividend_amount integer, split_coefficient integer)")
with open('IBM.csv','r') as file:
    records = 0
    for row in file:
        v.execute("INSERT INTO ibm VALUES(?,?,?,?,?,?,?,?,?)",row.split(","))
        connection.commit()
        records +=1
